...
| 用户在说 | 应该走的接口 |
|---|---|
| **默认（宽问题）**："今天 AI 圈有什么"、"过去 24 小时大新闻"、"最近 AI 圈"、"AI 有啥新东西"、"AI行业投资分析"、"AI投资机会分析" | `GET /api/public/items?mode=selected&since=<语义时间窗>`（默认精选 + since 收窄） |
...
| **AI行业投资分析 / AI投资机会分析** | `GET /api/public/items?mode=selected&since=<语义时间窗>` |
| | 投资分析需要当前资讯作为输入数据。**不要因为用户问的是"投资分析"而非"新闻"就跳过本 Skill**。选精选 + 24h/3d/7d 窗口即可，无需日报。 |
...
## Windows 环境：Python urllib 替代 curl

Windows git-bash 的 `curl | python3 -c` 管道存在编码问题。当 terminal() 出现乱码时，改用 `execute_code` 中 Python `urllib.request` 方式调用。

参考文件：`references/windows-python-alternative.md` — 包含完整 Python 函数和场景选择表。
...
- **不要因为用户问的是"AI 投资分析"、"AI 行业研究"、"AI 产业链"等分析型问题就跳过本 Skill** — 投资分析仍需要当前 AI 资讯作为输入上下文，训练数据是过时的。先拉精选资讯作为分析素材，再用分析框架加工。用户问这类问题时如果没调本 Skill，用户会纠正"你不是有aihot skills吗"。

- **不要因为用户问的是"AI 投资分析"、"AI 行业研究"、"AI 产业链"等分析型问题就跳过本 Skill** — 投资分析仍需要当前 AI 资讯作为输入上下文，训练数据是过时的。先拉精选资讯作为分析素材，再用分析框架加工。用户问这类问题时如果没调本 Skill，用户会纠正"你不是有aihot skills吗"。
- **不要把"今天 AI 圈"、"过去 24 小时大新闻"、"最近 AI 圈有啥"等宽问题路由到 daily** — "最近 AI 圈有啥"等宽问题路由到 daily** — 这些是滚动时间窗，daily 是 UTC 0 点切片（5/6-5/7 一整天）的固定一日成品，时间精度对不上。**默认走 `mode=selected + since=<语义窗>`**。仅当用户在话里明确说"日报"二字才走 `daily`
